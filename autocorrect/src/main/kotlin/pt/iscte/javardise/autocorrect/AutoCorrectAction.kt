...
import pt.iscte.javardise.ReplaceCommand
...
class AutoCorrectAction : Action {
    ...
    val types = mutableSetOf("byte", "short", "int", "long", "float", "double", "char", "boolean")
        ...
        editor.addCommandObserver { command, _, stack ->
            ...
            // method declaration added (check type)
                ...
                when (stmt) {
                    ...
                    is AssignExpr -> handleAssignTarget(stmt, stack)

                    // call statement added (check id)
                    is MethodCallExpr -> handledCallScopeAndId(stmt, stack)
                }

            } else if (command is ModifyCommand<*>)
                when (command.newElement) {
                    // variable reference was renamed
                    is NameExpr -> handleVarRefRename(command as ModifyCommand<NameExpr>, stack)
                    ...
                    is Type -> handleTypeRefRename(command as ModifyCommand<Type>, stack)
                }
        }
    }


    private fun handleAssignTarget(assign: AssignExpr, stack: CommandStack) {
        when (assign.target) {
            is NameExpr -> {
                try {
    ...
    private fun handledCallScopeAndId(call: MethodCallExpr, stack: CommandStack) {
    ...
    private fun handleMethodDecType(method: MethodDeclaration, stack: CommandStack) {
        try {
            method.type.resolve()
        } catch (e: UnsolvedSymbolException) {
    ...
    private fun handleTypeRefRename(command: ModifyCommand<Type>, stack: CommandStack) {
        try {
    ...
    private fun handleVarRefRename(command: ModifyCommand<NameExpr>, stack: CommandStack) {
        try {
